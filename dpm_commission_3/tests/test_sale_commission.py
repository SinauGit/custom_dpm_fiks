...
class TestSaleCommission(TestAccountCommission):
    ...
    def _create_sale_order(self, agent, commission):
        ...
        order.order_line.agent_ids = [
            (0, 0, {"agent_id": agent.id, "commission_id": dpm_commission.id})
        ]
        return order
    ...
    def test_sale_commission_gross_amount_payment(self):
        self._check_full(
            self.env.ref("dpm_commission.res_partner_pritesh_sale_agent"),
            ...
            (
                ...
                0,
                {
                    "agent_id": self.env.ref(
                        "dpm_commission.res_partner_pritesh_sale_agent"
                    ).id,
                    "commission_id": self.env.ref("dpm_commission.demo_commission").id,
                },
            ),
            (
                0,
                ...
                {
                    "agent_id": self.env.ref(
                        "dpm_commission.res_partner_eiffel_sale_agent"
                    ).id,
                    "commission_id": self.env.ref("dpm_commission.demo_commission").id,
                },
            ),
